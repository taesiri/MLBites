...
- **Encoder**: A two-layer MLP that maps input \(x\) to hidden representation, then projects to \(\mu\) and \(\log\sigma^2\) (mean and log-variance of the latent Gaussian).
- **Reparameterization trick**: Sample \(z = \mu + \sigma \cdot \epsilon\) where \(\sigma = \exp(0.5 \cdot \log\sigma^2)\) and \(\epsilon \sim \mathcal{N}(0,1)\). This allows gradients to flow through the sampling operation.
...
- **VAE loss** combines two terms:
  ...
  - KL divergence: regularizes the latent space to be close to a standard normal \(\mathcal{N}(0, I)\).
- The KL divergence for Gaussians has a closed form: \(-0.5 \cdot \sum(1 + \log\sigma^2 - \mu^2 - \sigma^2)\).
...
- The encoder outputs \(\log\sigma^2\) instead of \(\sigma^2\) or \(\sigma\) for numerical stability (log-space is unbounded and easier to optimize).
- KL divergence derivation assumes the prior is \(\mathcal{N}(0, I)\) and the posterior is diagonal Gaussian \(\mathcal{N}(\mu, \text{diag}(\exp(\log\sigma^2)))\).
...
- Forgetting to use \(\log\sigma^2\) (using variance directly leads to numerical issues with negative values).
- Using \(0.5 \cdot \log\sigma^2\) instead of \(\exp(0.5 \cdot \log\sigma^2)\) for \(\sigma\)—remember to exponentiate!
